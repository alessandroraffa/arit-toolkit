...
### [x] Activity 4: Add coverage and documentation for the idempotent flat-layout migration sweep
...
#### [x] Task 4.1: Add unit tests for flat-layout migration

Read `test/unit/features/agentSessionsArchiving/archiveService.test.ts` in full before making any change.

- [x] Add a new `describe('migrateFlatLayout (via deduplicateAndHydrate)', ...)` block after the existing `describe('reconfigure', ...)` block. The block contains five test cases. Import `FileType` from `../../mocks/vscode` at the top of the test file (it is already exported by `test/unit/mocks/vscode.ts` as an enum).
...
#### [x] Task 4.2: Update impacted documentation

- [x] In `docs/technical-context.md`, locate section 8.6 "Agent Session Archiving Model". Find the "One-shot re-archive on startup" paragraph. After that paragraph, add the following new paragraph:
...
- **Task 3.2/4.1 (FLAT_PATTERN regex digit count bug, fix-forward)**: the workstream-prescribed regex `/^(\d{4})(0[1-9]|1[0-2])\d{8}-.+\.\w+$/` for `migrateFlatLayout` requires `4 + 2 + 8 = 14` digits before the `-`, but flat archive filenames use the existing `YYYYMMDDHHmm` 12-digit timestamp (YYYY 4 + MM 2 + DDHHmm 6). With `\d{8}`, NO flat file ever matches, making `migrateFlatLayout` a permanent no-op and silently breaking the entire idempotent migration feature. The bug shipped in Activity 3's commit (Task 3.2 line — pre-existing wrong spec). Detected during Activity 4 test runs (all migration tests failed because copy was never called). Disposition: fix-forward in `src/features/agentSessionsArchiving/archiveService.ts`: changed `\d{8}` → `\d{6}` and added an inline comment explaining the digit budget. Production code now correctly matches `YYYYMMDDHHmm-...{ext}`. Activity 3's commit history retains the buggy regex (commits are append-only); Activity 4's commit ships the fix alongside the new migration tests that exercise the corrected path. The workstream's prose example in Task 3.2 also references `'202099310000-foo.md'` (12 digits) which contradicts the `\d{8}` digit count — internal inconsistency confirms the bug. Corrective: workstream authoring should test prescribed regexes against the actual filename format documented elsewhere in the same workstream BEFORE presenting; the inconsistency between the regex (14 digits) and the example string (12 digits) was a self-detectable contradiction at authoring time.
- **Task 4.1 (race-resilient redesign of migration tests)**: same `start()` fire-and-forget race as Task 3.4 / 3.5. The five migration tests (`should move a flat archive file into the YYYY/MM subdirectory`, `should be idempotent`, `should handle a mixed tree`, `should overwrite the destination if it already exists`, `should leave the source in place and continue when copy fails`) all suffered double-execution when both Cycle A (fire-and-forget) and Cycle B (test's awaited cycle) entered `migrateFlatLayout` concurrently. Disposition: every migration test now bypasses `start()` (sets `_currentConfig` and `_needsDedup = true` directly via a typed cast) and invokes `runArchiveCycle` once, ensuring a single sequential migration pass. This makes the tests deterministic and accurately exercise the per-cycle migration behavior. Corrective: a future workstream-authoring template should make the `start() fire-and-forget` race an explicit design consideration for any test that asserts call counts on `runArchiveCycle`-driven mutations.
...
- **Spec gap**: 9 (Tasks 1.6, 2.1, 2.5, 3.2/4.1 [FLAT_PATTERN regex bug], 3.4, 3.5, 3.7, 4.1 [race-resilient redesign of migration tests], and the cross-cutting bypass-`start()` pattern). Every divergence in this workstream traces back to workstream authoring choices that did not survive contact with the live codebase, the live tooling, or the live runtime semantics. No codebase drift, no convention ambiguity, no tooling limitation was observed. The pattern is overwhelming.
- **Codebase drift**: 0.
- **Convention ambiguity**: 0.
- **Tooling limitation**: 0.

**Pattern identified.** A single category accumulated nine divergences, all of authoring origin. The workstream was technically detailed but did not validate prescribed artifacts (regexes, commit subjects, mock patterns, file references) against the project's actual constraints (filename format, commitlint config, async runtime behavior, file inclusion required for the quality gate).

**Proposed improvements** (each lists target document + specific addition; all require PM authorization before being applied)

1. **Authoring checklist — commit message validation** (`skills/workstream-authoring/SKILL.md` or local `docs/project-context.md` § Configuration). Before prescribing a commit subject, validate against the project's `commitlint.config.*`: (a) header ≤ `header-max-length` (typically 100), (b) subject case complies with `subject-case` (typically lower-case — camelCase identifiers must be rephrased), (c) type ∈ `type-enum`. Caught two divergences in WS-0013 (Tasks 2.5, 3.7).
2. **Authoring checklist — regex sanity check** (`skills/workstream-authoring/SKILL.md`). When prescribing a regex with explicit digit counts (`\d{N}`), the workstream must include AT LEAST ONE positive example string matching the regex and verify the digit count by line-counting the example. Internal contradiction between the regex and the example (FLAT_PATTERN required 14 digits, example string had 12) was self-detectable at authoring time. Caught the silent feature-killing bug in WS-0013 Task 3.2.
3. **Authoring checklist — lint surface scoping** (`skills/workstream-authoring/SKILL.md`). The "files to commit" list for any activity that runs the project's lint gate must include every untracked or modified markdown sibling artifact (review reports, draft notes) that the lint glob will inspect. Caught the MD038 markdownlint failure in WS-0013 Task 1.6.
4. **Authoring checklist — forward reference resolution** (`skills/workstream-authoring/SKILL.md`). When a task references an anchor "introduced by a later task", the workstream must annotate placement against the state at the CURRENT task's commit time, not the post-future-task state. Specifically: insert anchors must exist at the time of execution. Caught the `_reconfiguring` field placement bug in WS-0013 Task 2.1.
5. **Authoring checklist — async/concurrency design for tests** (`skills/workstream-authoring/SKILL.md`). Whenever a service-under-test has fire-and-forget side effects from its public API (e.g., `start()` doing `void this.runArchiveCycle()`), tests that assert exact call counts on mocks consumed by that side effect must either (a) use `mockImplementation` keyed on the discriminator (e.g., `uri.fsPath`) instead of `mockResolvedValueOnce` chains, or (b) bypass the fire-and-forget by directly setting service state. The `mockResolvedValueOnce` chain pattern (prescribed for 8 of the 10 dedup tests in WS-0013 Task 3.5) is unsound when the production code emits multiple concurrent consumers of the same mock. Caught the test-design failure pattern in WS-0013 Tasks 3.4, 3.5, 4.1.

**Assessment**: pattern identified. Recurring spec-gap category indicates a systemic gap in workstream-authoring discipline: the artifact passes structural cross-verification (file paths exist, anchor lines match) but fails dynamic cross-verification against runtime semantics (regex matching, async ordering, lint gate scope, commitlint constraints). The five proposed improvements collectively address this gap. The Phase 2 multi-perspective review did not surface any of these issues — they are dynamic-execution defects invisible to artifact-level inspection.
