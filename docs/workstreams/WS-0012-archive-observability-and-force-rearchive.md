...
- [x] In `test/unit/features/agentSessionsArchiving/index.test.ts`, inside `describe('archive now command')`, add a new test case: `'should show information message after archive cycle completes'`. The test: sets `mockService.currentConfig = { enabled: true }`; registers the feature; retrieves the `tangyr.archiveAgentSessionsNow` handler; calls `await handler()`; asserts `window.showInformationMessage` was called with `'Agent sessions archive completed.'`.
