...
**D-1 (Activity 2, Task 2.2 — Test 7 `onDidChangeState` assertion, RESOLVED via post-completion amendment):** The workstream originally prescribed that Test 7 assert "`onDidChangeState` was fired at least once with value `true`". The executor observed that `verifyLegacyConfigMigration` Path A — as prescribed — called `applyConfig()` + `runMigration()`, neither of which fires `_onDidChangeState`. The executor removed the assertion as the test-only remediation and escalated to the PM. **PM resolution (post-completion amendment):** confirmed the omission as a defect, not a prescription error. In the user's target scenario the upstream `_onDidChangeState.fire()` at line 128 of `initialize()` is skipped (because `_isInitialized` is still `false` when the backstop fires), so feature services listening on `onDidChangeState` would never see the post-migration enable/disable transition. **Corrective action applied:** added `this._onDidChangeState.fire(this._isEnabled);` to Path A immediately after `applyConfig(parsed)` and `_loadedLegacyConfigFile = true`, mirroring the normal-flow pattern from `initialize()` line 128. Test 7 was updated to subscribe to `onDidChangeState` before invoking `initialize()` and to assert that the captured `stateChanges` array contains `true`. Quality gate post-fix: check-types ✓, lint ✓ (0 errors), test:unit ✓ (786/786 still passing). Recorded as a post-completion amendment commit on this branch rather than as a separate WS-0016, given the one-line scope and the direct PM correction of the original revision instruction.

### Reflection

**Divergence count by cause:** 1 total.

- **Workstream prescription defect (1, resolved):** D-1 — the revised Path A in Task 1.3 (per review F-7/F-8) dropped the explicit `_onDidChangeState.fire` call that the original draft had included. The executor surfaced the gap via the unreachable Test 7 assertion. PM confirmed the omission as a defect and amended Path A to fire `_onDidChangeState` after `applyConfig`. Root cause: the revision pass conflated the `notifySectionListeners` concern (per F-8) with the `_onDidChangeState` concern, removing both even though `applyConfig` only handles the former.
