...
Features receive a `FeatureRegistrationContext` from `src/features/index.ts`. Functions with >3 params must use a context/deps object instead:

```typescript
// src/features/myFeature/index.ts
export function registerMyFeature(ctx: FeatureRegistrationContext): void {
...
|------|-------|------------------|
| `max-lines` | 250 | Small, focused files |
| `max-lines-per-function` | 50 | Small, single-purpose functions |
| `complexity` | 10 | Low cyclomatic complexity (max 10 independent paths) |
| `max-depth` | 3 | Shallow nesting (use early returns and guard clauses) |
| `max-nested-callbacks` | 3 | Flat callback structure |
| `max-params` | 3 | Use context/deps objects for >3 params |
| `max-statements` | 15 | Concise function bodies |
| `max-classes-per-file` | 1 | One class per file (error, not warning) |

When a function approaches these limits, extract helpers, use early returns, or split into smaller modules.

### Test files (`test/**/*.ts`)

Linted with `@vitest/eslint-plugin` recommended rules plus relaxed limits:

- `max-lines`: 400 (tests need more space)
- `max-lines-per-function`: off (`describe`/`it` blocks)
- `max-statements`: off (test setup)
- `max-nested-callbacks`: 4 (`describe > describe > it > callback`)
- `no-explicit-any`: off (mock casts) — do NOT add `eslint-disable` comments for `any` in tests
- Vitest rules: `expect-expect`, `no-focused-tests`, `no-disabled-tests`, `no-identical-title`, etc.
...
- Keep files under 250 lines, functions under 50 lines, params at 3 or fewer
...
- Use `any` type in source files (use `unknown` + type narrowing)
...
- Add `eslint-disable` comments for `no-explicit-any` in test files (the rule is already off)
